...
def main():
    ...
    for i in range(args.nmodels):
        ...
        if args.lens:
            ...
            multiple_images = []
            for k, row in df_src.iterrows():
                kwargs_src = {'ys1': row['x'], 'ys2': row['y'], 'flux': 1.0, 'zs': row['zgal']}
                df.change_redshift(row['zgal'])
                ps = pointsrc(size=args.fieldsize, Npix=a1.shape[0], gl=df, use_lenstronomy=False, refine=True, **kwargs_src)
                xi, yi, mui = ps.find_images()
                mags_ = -2.5 * np.log10(np.abs(mui)) + row['mag']
                # sort images by magnitude
                xi, yi, mui, mags_ = zip(*sorted(zip(xi, yi, mui, mags_), key=lambda x: x[3]))
                xi, yi, mui, mags_ = np.array(xi), np.array(yi), np.array(mui), np.array(mags_)

                # select images with magnitude below the limit
                imag = mags_ < args.img_maglim

                if (len(xi)>1) and np.sum(imag)>1:
                    for kk in range(len(xi)):
                        new_ima ={
                            "ID": f"{k+1}.{kk+1}",  # ID with multiplicity
                            "RA": xi[kk],
                            "DEC": yi[kk],
                            "a": 1.0,
                            "b": 1.0,
                            "theta": 0.0,
                            "z": kwargs_src['zs'],
                            "mag": -2.5 * np.log10(np.abs(mui[kk])) + row['mag']  # Convert flux to magnitude
                        }
                        multiple_images.append(new_ima)
                        if args.plotimages:
                            ax.plot(xi[kk], yi[kk], '+', color='white', markersize=8)
                            ax.text(xi[kk] + 0.1, yi[kk] + 0.1, f"{k + 1}.{kk + 1}", fontsize=8, color='white', ha='center', va='center')
            plt.tight_layout()
            ...
            with open(output_csv, "w") as f:
                ...
                for entry in multiple_images:
                    if entry['mag'] < args.img_maglim:
                        nsaved += 1
                        # Only save images with magnitude below the limit
                        f.write(
